  ...
  return (
    <div className="max-w-md mx-auto mt-10 card">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <TextField label="Password" name="password" value={password} onChange={setPassword} placeholder="Min 8 chars, 1 upper, 1 lower, 1 number" type="password" autoComplete="new-password" error={errors.password} />
        <div>
          <label className="label">Role</label>
          <div className="flex gap-3">
            <label className="badge cursor-pointer">
              <input className="mr-2" type="radio" name="role" value="readOnly" checked={role==='readOnly'} onChange={() => setRole('readOnly')} />
              readOnly
            </label>
            <label className="badge cursor-pointer">
              <input className="mr-2" type="radio" name="role" value="readWrite" checked={role==='readWrite'} onChange={() => setRole('readWrite')} />
              readWrite
            </label>
          </div>
        </div>
      ...
      <p className="mt-4 text-sm">Already have an account? <Link className="link" to="/login">Sign in</Link></p>
    </div>
  )
}
